...
// порог автоматического включения вентилятора (на сколько влажность внутри гаража больше приведенной уличной)
#define deltaHumidity 10 
// разница влажности между включением и выключением вентилятора. Чтобы реле 220V не щелкало слишком часто
#define hysteresis 1                      
...
void build(gh::Builder& b) {      // билдер GyverHub.
  ...
  if (b.beginRow()) {
    if (b.Spinner(&hum4xCorrection).range(-10, 10, 1).label(F("Поправка влажности на улице")).click()) {
    ...
    if (b.Spinner(&hum3xCorrection).range(-10, 10, 1).label(F("Поправка влажности в гараже")).click()) {
...
void loop() {
  ...
  if (ventAuto && carStatus && (humidityGarage - humidityCalc >= deltaHumidity + hysteresis)) {
  ...
  if (ventAuto && (humidityGarage - humidityCalc < deltaHumidity - hysteresis)) {
